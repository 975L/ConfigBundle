...
class Config
{
    ...
    // Restricts the config (regardless of group) to ROLE_SUPER_ADMIN only, for secrets shared across an install (backup DB credentials, payment API keys...) that a regular site admin must never see
    #[ORM\Column(type: Types::BOOLEAN, nullable: true)]
    #[Assert\Type(type: 'bool')]
    ...
    // Any character valid in a CSS <color> (hex, rgb()/rgba(), hsl()/hsla(), named colors like "tomato"): also blocks ";", "{", "}", quotes... so a theme color can't break out of its declaration in the site-theme.css file generated by SiteBundle's ThemeVariablesCssListener
    private const THEME_COLOR_PATTERN = '/^[#a-zA-Z0-9.,%()\s-]+$/';

    // Theme slugs holding a CSS color value (as opposed to a font stack or the theme-mode choice)
    private const THEME_COLOR_SLUG_PREFIX = 'theme-color-';

    // Validates that a "theme-color-*" config (site_config, group "theme") holds a plausible CSS color
    #[Assert\Callback]
    public function validateThemeColorValue(ExecutionContextInterface $context): void
    {
        if (self::GROUP_THEME === $this->group
            ...
            && null !== $this->value
            && '' !== $this->value
            && 1 !== preg_match(self::THEME_COLOR_PATTERN, $this->value)
        ) {
            $context->buildViolation('label.invalid_theme_color')
                ->atPath('value')
                ->addViolation();
        }
    }
