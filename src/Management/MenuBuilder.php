    ...
    public function getMenuItems(): iterable
    {
        ...
        if ([] !== $links) {
            ...
            foreach ($links as $link) {
                // "url" (a literal, already-absolute URL) takes precedence when a provider sets it -
                // for a link a provider wants fixed/directly editable rather than derived from a route
                // (e.g. pointing at a specific known deployment on purpose). Otherwise "name" is a route
                // name resolved via linkToUrl()+the plain router, not linkToRoute(): the latter resolves
                // through EasyAdmin's own AdminUrlGenerator (see MenuFactory), which assumes the route is
                // one of the dashboard's own registered actions and wraps it as "/management?routeName=
                // ...&..." - correct only for a route reachable *through* the dashboard. A link to a
                // plain, unrelated route (e.g. a consuming app's own public page) needs its real path
                // instead, which the plain router already gives via generate() - this works uniformly for
                // both cases, since a dashboard-registered route's own real path resolves correctly
                // through generate() too.
                //
                // A route-based link with a "target" (see below) is leaving the admin entirely - resolved
                // as a full absolute URL (scheme+host), not just a path, since it's meant to stand on its
                // own once opened in a new tab. Still generated from the current request each time, never
                // hardcoded, so it stays correct across dev/staging/prod or any future domain change -
                // only the route itself is a fixed reference, the same way a same-tab link already works.
                if (isset($link['url'])) {
                    $linkUrl = $link['url'];
                } else {
                ...
                // Optional per-link role (see MenuProviderInterface::getLinks()) - unlike CRUD menus above,
                // links can point to routes needing anything from a public page to an editor-only one, so
                // there's no single sensible default; providers not needing gating simply omit the key
                if (isset($link['role'])) {
                    $item->setPermission($link['role']);
                }

                // Optional link target (e.g. '_blank' for a link leaving the admin - see
                // MenuProviderInterface::getLinks()) - management.scss shows an external-link glyph
                // automatically for any target="_blank" link, no per-provider styling needed
